...
// 10.-Crear Backup ----> http://localhost:3000/api/Backup
...
router.post('/Backup', (req, res) => {
  ...
  mysqlBackup({
    ...
    fs.writeFileSync(ruta + '/FeyAlegriaBackup.sql', dump); // RUTA + NOMBRE DEL ARCHIVO SQL
  });
  res.json({ status: 'ok' });
});

// 11.-RESTORE ----> http://localhost:3000/api/Restore
// RECIBE RUTA DESTINO de donde esta ubicado el respaldo
router.post('/Restore', (req, res) => {
  const { ruta } = req.body;
  const database = 'FeyAlegria'; // FeyAlegria
...
// 12.-CREAR DB ----> http://localhost:3000/api/Restore
// RECIBE RUTA DESTINO de donde esta ubicado el respaldo
