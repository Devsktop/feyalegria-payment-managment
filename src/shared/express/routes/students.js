...
router.get('/students/:section', async (req, res) => {
  ...
  const { students, errGetStudentsBySection } = await getStudentsBySection(
    section,
    pag,
    pattern
  );
...
//   WHERE representatives.idRepresentative = students.idRepresentative
//   AND students.idSection = ${section};`;
